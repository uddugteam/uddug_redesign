...
export type ScreenSize = 'desktop' | 'tablet-landscape' | 'tablet-vertical' | 'mobile'

export const useScreenSize = (): ScreenSize => {

  const [currentScreenSize, setCurrentScreenSize] = useState<ScreenSize>('mobile')

  const isMobile = useMedia('(max-width: 719px)', false)
  const isTabletVertical = useMedia('(min-width: 720px) and (max-width: 1023px)', false)
  const isTabletLandscape = useMedia('(min-width: 1024px) and (max-width: 1279px)', false)
